...
# ============================================
# DESIGN DNA EXTRACTION SYSTEM
# ============================================

def extract_colors_from_css(css_text):
    """Extrae colores hex/rgb del CSS"""
    import re
    colors = []
...
def extract_design_dna_from_url(url, timeout=10):
    """Extrae Design DNA de una URL competidora"""
    try:
        headers = {
        ...
        # Extraer colores de estilos inline y CSS
        style_tags = soup.find_all('style')
        for style in style_tags:
            colors = extract_colors_from_css(style.get_text())
            design_dna['colors'].extend(colors)
        
        # Extraer colores de atributos style
        elements_with_style = soup.find_all(style=True)
        for elem in elements_with_style[:50]:  # Limitar a 50
            style_attr = elem.get('style', '')
            colors = extract_colors_from_css(style_attr)
            design_dna['colors'].extend(colors)
        
        # Extraer fuentes
        for elem in soup.find_all(['h1', 'h2', 'h3', 'p', 'div'])[:30]:
            style = elem.get('style', '')
            if 'font-family' in style:
                import re
                font_match = re.search(r'font-family:\s*([^;]+)', style)
                if font_match:
                    design_dna['fonts'].append(font_match.group(1).strip())
        
        # Detectar secciones comunes
        page_text = soup.get_text().lower()
        
        if any(keyword in page_text for keyword in ['testimonial', 'cliente', 'opinion', 'review', 'valoracion']):
            design_dna['has_testimonials'] = True
        
        if any(keyword in page_text for keyword in ['pregunta', 'faq', 'frecuente', 'duda']):
            design_dna['has_faq'] = True
        
        if any(keyword in page_text for keyword in ['precio', 'tarifa', 'coste', 'presupuesto', 'price', 'pricing']):
            design_dna['has_pricing'] = True
        
        if any(keyword in page_text for keyword in ['galeria', 'trabajo', 'portfolio', 'proyecto', 'gallery']):
            design_dna['has_gallery'] = True
        
        # Detectar gradientes
        if 'gradient' in response.text.lower():
            design_dna['has_gradient'] = True
        
        # Extraer textos de CTAs (botones)
        buttons = soup.find_all(['button', 'a'], class_=lambda x: x and any(btn in str(x).lower() for btn in ['btn', 'button', 'cta']))
        for btn in buttons[:10]:
            text = btn.get_text().strip()
            if text and len(text) < 50:
                design_dna['cta_texts'].append(text)
        
        # Extraer tamaños de headings
        for tag in ['h1', 'h2', 'h3']:
            ...
            for elem in elements[:5]:
                ...
                if 'font-size' in style:
                    ...
                    if size_match:
                        design_dna['heading_sizes'].append({
                            'tag': tag,
                            'size': size_match.group(1).strip()
                        })
        
        # Detectar estructura de secciones (por headers)
        sections_detected = []
        for h2 in soup.find_all('h2')[:10]:
            section_title = h2.get_text().strip().lower()
            if section_title:
                sections_detected.append(section_title)
        
        design_dna['sections'] = sections_detected
...
def consolidate_design_dna(dna_list):
    ...
    consolidated = {
        ...
        'common_ctas': [],
        'common_sections': [],
        'competitors_analyzed': len(dna_list)
    }
    
    # Consolidar colores (los más frecuentes)
    all_colors = []
    for dna in dna_list:
        if dna:
            all_colors.extend(dna.get('colors', []))
    
    # Filtrar solo colores hex válidos de 6 dígitos
    import re
    valid_hex = [c for c in all_colors if re.match(r'^#[0-9a-fA-F]{6}
